...
class TestShouldRejectHtmlResponse:
    ...
    def test_html_for_epub_url_is_rejected(self) -> None:
        reject, reason = _should_reject_html_response(
            "text/html", "https://example.org/file.epub"
        )
        assert reject is True

    def test_html_for_download_url_is_rejected(self) -> None:
        reject, reason = _should_reject_html_response(
            "text/html", "https://example.org/download?id=123"
        )
        assert reject is True

    def test_html_for_regular_url_is_accepted(self) -> None:
        reject, reason = _should_reject_html_response(
            "text/html", "https://example.org/about"
        )
        assert reject is False

    def test_annas_archive_login_page(self) -> None:
        reject, reason = _should_reject_html_response(
            "text/html",
...
class TestValidateFileMagicBytes:
    ...
    def test_invalid_pdf_with_html(self, tmp_path: Path) -> None:
        f = tmp_path / "test.pdf"
    ...
    def test_valid_epub(self, tmp_path: Path) -> None:
        f = tmp_path / "test.epub"
        f.write_bytes(b"PK\x03\x04 test content")
        valid, msg = _validate_file_magic_bytes(str(f), ".epub")
        assert valid is True

    def test_invalid_epub_with_html(self, tmp_path: Path) -> None:
        f = tmp_path / "test.epub"
        f.write_bytes(b"<!DOCTYPE html><html><body>Error</body></html>")
        valid, msg = _validate_file_magic_bytes(str(f), ".epub")
        assert valid is False

    def test_non_pdf_epub_always_valid(self, tmp_path: Path) -> None:
        f = tmp_path / "test.jpg"
...
class TestValidateHtmlNotLoginPage:
    ...
    def test_non_annas_archive_accepted(self, tmp_path: Path) -> None:
    ...
    def test_annas_archive_login_detected(self, tmp_path: Path) -> None:
        f = tmp_path / "page.html"
        f.write_text("<html><title>log in / register</title></html>")
    ...
    def test_annas_archive_normal_page_accepted(self, tmp_path: Path) -> None:
        f = tmp_path / "page.html"
...
class TestDetermineTargetDirectory:
    ...
    def test_disallowed_ext_without_metadata_save(self) -> None:
        target, msg, success = _determine_target_directory(
            "/work", ".html", [".pdf"], False
        )
    ...
    def test_disallowed_ext_with_metadata_save(self) -> None:
        target, msg, success = _determine_target_directory(
            "/work", ".html", [".pdf"], True
        )
...
class TestSaveJson:
    ...
    @patch("api.core.download.get_current_name_stem", return_value="test_stem")
    @patch("api.core.download.get_current_provider", return_value="ia")
    ...
    @patch("api.core.download.include_metadata", return_value=False)
    def test_skips_when_metadata_disabled(
        self, mock_meta: MagicMock, tmp_path: Path
    ) -> None:
