...
class TestRunIdentifierCLI:
    ...
    def test_iiif_provider_calls_process_direct_iiif(
        ...
        assert "bsb11280551" in call_kwargs.kwargs.get(
            "manifest_url", call_kwargs[1].get("manifest_url", "")
        ) or "bsb11280551" in str(call_kwargs)
    ...
    def test_name_passed_through(self, mock_process: MagicMock) -> None:
        ...
        assert (
            call_kwargs[1].get("title") == "Kochbuch"
            or call_kwargs.kwargs.get("title") == "Kochbuch"
        )
    ...
    def test_dry_run_passed_through(self, mock_process: MagicMock) -> None:
        ...
        call_kwargs = mock_process.call_args
        assert (
            call_kwargs[1].get("dry_run") is True
            or call_kwargs.kwargs.get("dry_run") is True
        )
    ...
    def test_unknown_provider_logs_error(
    ...
    def test_unrecognised_id_no_provider_logs_error(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    @patch(
        "api.identifier_resolver.PROVIDERS",
        {
            "annas_archive": (
                MagicMock(),
