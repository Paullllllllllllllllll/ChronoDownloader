...
class TestConsoleUIPrintMethods:
    ...
    def test_print_info_with_message(self, capsys):
        ConsoleUI.print_info("Label", "Message text")
        output = capsys.readouterr().out
        assert "Label" in output
        assert "Message text" in output

    def test_print_info_label_only(self, capsys):
    ...
    def test_print_session_summary_with_duration_seconds(self, capsys):
        ConsoleUI.print_session_summary(
    ...
    def test_print_session_summary_with_duration_minutes(self, capsys):
        ConsoleUI.print_session_summary(
            processed=5, succeeded=5, failed=0, deferred=0,
    ...
    def test_print_session_summary_with_duration_hours(self, capsys):
        ConsoleUI.print_session_summary(
            processed=5, succeeded=5, failed=0, deferred=0,
    ...
    def test_print_session_summary_with_deferred_shows_next_steps(self, capsys):
        ConsoleUI.print_session_summary(
...
class TestConsoleUIPromptMethods:
    ...
    def test_prompt_yes_no_explicit_yes(self):
        with patch("builtins.input", return_value="y"):
            assert ConsoleUI.prompt_yes_no("Continue?") is True

    def test_prompt_yes_no_explicit_no(self):
...
class TestConsoleUIEnableAnsi:
    ...
    def test_enable_ansi_on_windows(self):
        # Should not raise even if ctypes fails
        with patch("builtins.__import__", side_effect=ImportError):
            ConsoleUI.enable_ansi()

    @patch("sys.platform", "linux")
    def test_enable_ansi_noop_on_linux(self):
        ConsoleUI.enable_ansi()
