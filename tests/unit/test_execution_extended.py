...
class TestGetDirectLink:
    ...
    def test_returns_iiif_url_from_direct_link_column(self):
        row = pd.Series({
    ...
    def test_returns_iiif_url_from_link_column(self):
        row = pd.Series({
    ...
    def test_returns_none_for_non_iiif_url(self):
        row = pd.Series({
...
class TestProcessDirectIIIF:
    ...
    @patch("main.execution.preview_manifest")
    ...
    @patch("main.execution.preview_manifest")
    def test_dry_run_returns_failed_on_bad_manifest(self, mock_preview):
    ...
    @patch("main.execution.download_from_iiif_manifest")
    @patch("main.work_manager.compute_work_dir", return_value=("/out/work", "work_name"))
    def test_successful_download(self, mock_dir, mock_dl):
    ...
    @patch("main.execution.download_from_iiif_manifest")
    @patch("main.work_manager.compute_work_dir", return_value=("/out/work", "work_name"))
    def test_failed_download(self, mock_dir, mock_dl):
...
class TestSetupBackgroundScheduler:
    ...
    @patch("main.execution.get_background_scheduler")
    def test_returns_none_when_disabled(self, mock_sched):
        import logging
...
class TestRunSequential:
    ...
    @patch("main.execution.is_direct_download_enabled", return_value=False)
    @patch("main.execution.pipeline")
    def test_processes_works_sequentially(self, mock_pipeline, mock_direct):
        import logging
    ...
    @patch("main.execution.is_direct_download_enabled", return_value=False)
    @patch("main.execution.pipeline")
    def test_skips_rows_without_title(self, mock_pipeline, mock_direct):
        import logging
    ...
    @patch("main.execution.is_direct_download_enabled", return_value=False)
    @patch("main.execution.pipeline")
    def test_skips_rows_without_entry_id(self, mock_pipeline, mock_direct):
        import logging
    ...
    @patch("main.execution.is_direct_download_enabled", return_value=False)
    @patch("main.execution.pipeline")
    def test_updates_csv_on_success(self, mock_pipeline, mock_direct):
        import logging
        ...
        with patch("main.execution.mark_success", return_value=True) as mock_mark:
    ...
    def test_stops_on_budget_exhausted(self, mock_utils, mock_pipeline, mock_direct):
        import logging

        mock_utils.budget_exhausted.return_value = True
        mock_pipeline.process_work.return_value = {"status": "completed", "item_url": "", "provider": ""}
        works_df = pd.DataFrame({
            "short_title": ["Book A", "Book B"],
...
class TestRunBatchDownloads:
    ...
    @patch("main.execution._run_sequential")
    @patch("main.execution.get_deferred_queue")
    def test_sequential_mode(self, mock_queue, mock_seq):
    ...
    @patch("main.execution._run_sequential")
    @patch("main.execution.get_deferred_queue")
    def test_dry_run_uses_sequential(self, mock_queue, mock_seq):
...
class TestCreateInteractiveCallbacks:
    ...
    def test_returns_callback_pair(self, mock_ui):
        import logging
    ...
    @patch("main.console_ui.ConsoleUI")
    def test_submit_callback_increments_counter(self, mock_ui):
        import logging
    ...
    @patch("main.console_ui.ConsoleUI")
    def test_complete_callback_handles_success(self, mock_ui):
        import logging
    ...
    @patch("main.console_ui.ConsoleUI")
    def test_complete_callback_handles_failure(self, mock_ui):
        import logging
