...
from main.data.works_csv import ENTRY_ID_COL, LINK_COL, STATUS_COL
...
class TestParallelExecutionCsvSync:
    ...
    def test_csv_sync_with_custom_callback_success(
        self, temp_dir: str, test_csv_file: str, mock_task: MagicMock
    ) -> None:
        ...
        with (
            patch("main.orchestration.execution.pipeline") as mock_pipeline,
            ...
            patch("main.orchestration.execution.mark_failed"),
            patch(
                "main.orchestration.execution.get_parallel_download_config"
            ) as mock_config,
        ):
            ...
            mock_scheduler_instance.get_stats.return_value = {
                "completed": 1,
                "succeeded": 1,
                "failed": 0,
            }
            ...
            assert custom_callback_called.is_set(), (
                "Custom callback should be called after CSV sync"
            )

    def test_csv_sync_with_custom_callback_failure(
        self, temp_dir: str, test_csv_file: str, mock_task: MagicMock
    ) -> None:
        ...
        with (
            patch("main.orchestration.execution.pipeline") as mock_pipeline,
            patch("main.orchestration.execution.DownloadScheduler") as MockScheduler,
            patch("main.orchestration.execution.mark_success"),
    ...
    def test_csv_sync_without_custom_callback(
        self, temp_dir: str, test_csv_file: str, mock_task: MagicMock
    ) -> None:
    ...
    def test_csv_sync_skipped_without_csv_path(
        self, temp_dir: str, mock_task: MagicMock
    ) -> None:
    ...
    def test_csv_sync_skipped_without_entry_id(
        self, temp_dir: str, test_csv_file: str
    ) -> None:
...
class TestInteractiveCallbacksCsvSync:
    ...
    def test_create_interactive_callbacks_csv_sync_preserved(
        self, temp_dir: str
    ) -> None:
        ...
        with (
            ...
            # This is the key assertion: CSV sync must happen even with
