...
class TestWorkContext:
    ...
    def test_clear_work_id(self) -> None:
        """Test clearing work ID."""
        set_current_work("work_123")
        clear_current_work()
        assert get_current_work() is None

    def test_work_id_none_by_default(self) -> None:
        """Test that work ID is None by default."""
        clear_current_work()
        assert get_current_work() is None

    def test_overwrite_work_id(self) -> None:
...
class TestEntryContext:
    ...
    def test_clear_entry_id(self) -> None:
...
class TestNameStemContext:
    ...
    def test_clear_name_stem(self) -> None:
...
class TestProviderContextManager:
    ...
    def test_clears_provider_on_exit(self) -> None:
        """Test that provider is cleared when exiting context."""
        with provider_context("internet_archive"):
            pass
        assert get_current_provider() is None

    def test_clears_provider_on_exception(self) -> None:
...
class TestThreadIsolation:
    ...
    def test_different_threads_have_separate_context(self) -> None:
        ...
        def thread_task(thread_id: str) -> None:
    ...
    def test_counters_are_thread_local(self) -> None:
