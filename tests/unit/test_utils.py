...
class TestInferExtensionFromContentType:
    ...
    def test_epub_content_type(self) -> None:
...
class TestShouldRejectHtmlResponse:
    ...
    def test_rejects_html_for_pdf_url(self) -> None:
    ...
    def test_rejects_html_for_download_url(self) -> None:
...
class TestValidateFileMagicBytes:
    ...
    def test_invalid_pdf_with_html(self, temp_dir: str) -> None:
    ...
    def test_skips_non_pdf_epub(self, temp_dir: str) -> None:
...
class TestDetermineTargetDirectory:
    ...
    def test_filters_disallowed_extensions(self) -> None:
...
class TestSaveJson:
    ...
    def test_respects_include_metadata_false(self, temp_dir: str) -> None:
        """Test that save is skipped when include_metadata is False."""
        from api.core.download import save_json

        with patch("api.core.download.include_metadata", return_value=False):
            result = save_json({"key": "value"}, temp_dir, "metadata")
...
class TestDownloadIiifRenderings:
    ...
    def test_skips_when_disabled(self, temp_dir: str) -> None:
        ...
        with patch(
            ...
            return_value={"download_manifest_renderings": False},
        ):
            count = download_iiif_renderings(manifest, temp_dir)

        assert count == 0

    def test_respects_whitelist(self, temp_dir: str) -> None:
    ...
    def test_respects_max_limit(self, temp_dir: str) -> None:
