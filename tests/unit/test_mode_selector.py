...
class TestDetectModeAndParseArgs:
    ...
    def test_config_path_override(
        self, sample_config: dict[str, Any], temp_dir: str
    ) -> None:
...
class TestGetGeneralConfig:
    ...
    def test_returns_general_section(self, sample_config: dict[str, Any]) -> None:
        ...
        with patch("main.ui.mode.get_config", return_value=sample_config):
            ...
            assert (
                gen["interactive_mode"] == sample_config["general"]["interactive_mode"]
            )
            assert (
                gen["default_output_dir"]
                == sample_config["general"]["default_output_dir"]
            )
