...
class TestLoadWorksCsv:
    ...
    def test_required_columns_present(self, sample_csv_file: str) -> None:
...
class TestGetPendingWorks:
    ...
    def test_returns_false_status_works(self, sample_csv_data: pd.DataFrame) -> None:
    ...
    def test_excludes_completed_works(self, sample_csv_data: pd.DataFrame) -> None:
...
class TestGetCompletedEntryIds:
    ...
    def test_excludes_pending_ids(self, sample_csv_data: pd.DataFrame) -> None:
        """Test that pending IDs are excluded."""
        completed = get_completed_entry_ids(sample_csv_data)
        assert "E0001" not in completed
        assert "E0004" not in completed

    def test_returns_set(self, sample_csv_data: pd.DataFrame) -> None:
...
class TestGetStats:
    ...
    def test_returns_dict(self, sample_csv_file: str) -> None:
    ...
    def test_handles_missing_status_column(self, temp_dir: str) -> None:
