...
class TestProviderQuota:
    ...
    def test_get_reset_time_calculates_correctly(self) -> None:
        ...
        quota = ProviderQuota(
            provider_key="test", reset_hours=6, period_start=start_time.isoformat()
        )

        reset_time = quota.get_reset_time()

        expected = datetime(2024, 1, 1, 18, 0, 0, tzinfo=UTC)
    ...
    def test_seconds_until_reset_future(self) -> None:
        ...
        quota = ProviderQuota(
            provider_key="test", reset_hours=12, period_start=future_start.isoformat()
        )
    ...
    def test_seconds_until_reset_past(self) -> None:
        ...
        quota = ProviderQuota(
            provider_key="test", reset_hours=24, period_start=past_start.isoformat()
        )
    ...
    def test_is_exhausted_false_when_available(self) -> None:
        ...
        quota = ProviderQuota(provider_key="test", daily_limit=10, downloads_used=5)
    ...
    def test_is_period_expired(self) -> None:
        ...
        quota = ProviderQuota(
            provider_key="test", reset_hours=24, period_start=old_start.isoformat()
        )
    ...
    def test_reset_all_resets_all_providers(self, manager: Any) -> None:
        ...
        for i in range(3):
            manager._quotas[f"provider_{i}"] = ProviderQuota(
                provider_key=f"provider_{i}", downloads_used=i + 1
            )
