...
def _select_best_title(
    query_title: str, candidates: list[str]
...
def _build_annas_result(
    ...
    Constructs the raw result dict, converts it to a SearchResult, and
    attaches the per-title matching scores. When ``creators`` is non-empty it
    is also assigned to ``sr.creators``.
    """
    raw = {
        "title": best_title,
        ...
        if table_rows and len(table_rows) > 1:
            ...
            for row in table_rows[1:]:
                ...
                try:
                    ...
                    if len(md5_part) != 32 or not all(
                        c in "0123456789abcdefABCDEF" for c in md5_part
                    ):
                        continue
            ...
            for link in soup.find_all("a", href=True):
                ...
                if "/md5/" in href:
                    try:
                        ...
                        if len(parts) > 1:
                            ...
                            if len(md5_part) == 32 and all(
                                ...
                                snippets = [
                                    ...
                                    str(title_attr) if title_attr else "",
                                ]
                        ...
                        logger.debug(
                            "Error parsing Anna's Archive link %s: %s", href, e
                        )
                        continue
...
def _download_with_api(md5: str, api_key: str, output_folder: str) -> bool:
    ...
    try:
        ...
        if quota_info:
            logger.info(
                "Fast downloads remaining today: %s",
                quota_info.get("remaining", "unknown"),
            )
...
def _download_via_scraping(md5: str, output_folder: str) -> bool:
    ...
    for link in soup.find_all("a", href=True):
        ...
        is_direct_file = any(
            url_lower.endswith(ext) or f"{ext}?" in url_lower for ext in file_extensions
        )

        # Check if this is the slow download link
        if "slow_download" in href or "slow download" in text:
            slow_download_links.append(
                {"url": full_url, "source": text or "slow download"}
            )
        elif is_direct_file:
            direct_file_links.append({"url": full_url, "source": text or "direct file"})
        elif any(
            mirror in url_lower
            for mirror in ["libgen", "library.lol", "z-lib", "zlibrary", "ipfs"]
        ):
    ...
    logger.info(
        "Anna's Archive: Found %d direct, %d slow, %d mirror links for %s",
        len(direct_file_links),
        len(slow_download_links),
        len(mirror_links),
        md5,
    )
    ...
    for link_info in all_links:
        ...
        try:
            ...
            if result:
                any_downloaded = True
                logger.info(
                    "Successfully downloaded from Anna's Archive source: %s", source
                )
                break
        except Exception as e:
