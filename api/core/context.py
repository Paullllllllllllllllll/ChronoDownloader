...
def peek_counter(key: tuple[str, str, str]) -> int:
    ...
    Used by the early file-exists check to predict filenames without
    side effects on the counter state.

    Args:
        key: Tuple of (stem, provider_slug, type_key)

    Returns:
        What the counter value would be on the next increment
    """
    counters = get_counters()
