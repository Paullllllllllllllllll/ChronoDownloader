...
class DownloadConfiguration:
    ...
    max_workers_override: int | None = (
        None  # Optional override for max_parallel_downloads
    )
...
class ConsoleUI:
    ...
    @staticmethod
    def prompt_select(
        question: str, options: list[tuple[str, str]], allow_back: bool = True
    ) -> str | None:
        ...
        for i, (_value, display) in enumerate(options, 1):
        ...
        while True:
            try:
                choice = (
                    input(f"\n{ConsoleUI.BOLD}Enter choice: {ConsoleUI.RESET}")
                    .strip()
                    .lower()
                )
                ...
                print(
                    f"{ConsoleUI.YELLOW}Invalid choice. Please try again."
                    f"{ConsoleUI.RESET}"
                )
    ...
    def prompt_input(prompt: str, default: str = "", required: bool = False) -> str:
        ...
        while True:
            try:
                value = input(
                    f"{ConsoleUI.BOLD}{prompt}{default_hint}{required_hint}: "
                    f"{ConsoleUI.RESET}"
                ).strip()
    ...
    def prompt_yes_no(question: str, default: bool = True) -> bool:
        ...
        while True:
            try:
                answer = (
                    input(f"{ConsoleUI.BOLD}{question} {hint}: {ConsoleUI.RESET}")
                    .strip()
                    .lower()
                )
    ...
    def print_session_summary(
        ...
        if dry_run:
            print(
                f"    {ConsoleUI.YELLOW}* Mode: DRY RUN (no files downloaded)"
                f"{ConsoleUI.RESET}"
            )
        ...
        if succeeded > 0:
            print(
                f"    {ConsoleUI.GREEN}*{ConsoleUI.RESET} Succeeded: "
                f"{ConsoleUI.GREEN}{succeeded}{ConsoleUI.RESET}"
            )
        else:
        ...
        if failed > 0:
            print(
                f"    {ConsoleUI.RED}*{ConsoleUI.RESET} Failed: "
                f"{ConsoleUI.RED}{failed}{ConsoleUI.RESET}"
            )
        else:
        ...
        if deferred > 0:
            print(
                f"    {ConsoleUI.YELLOW}*{ConsoleUI.RESET} Deferred (quota): "
                f"{ConsoleUI.YELLOW}{deferred}{ConsoleUI.RESET}"
            )
        else:
        ...
        if duration_seconds is not None:
            if duration_seconds >= 3600:
                ...
                print(
                    f"    {ConsoleUI.CYAN}*{ConsoleUI.RESET} Duration: "
                    f"{hours:.1f} hours"
                )
        ...
        if deferred > 0:
            ...
            ConsoleUI.print_separator(".", width)
            print(
                f"    {ConsoleUI.DIM}* Run again to retry deferred downloads "
                f"when quotas reset{ConsoleUI.RESET}"
            )
            print(
                f"    {ConsoleUI.DIM}* Use --quota-status to check quota status"
                f"{ConsoleUI.RESET}"
            )
            ConsoleUI.print_separator(".", width)
        ...
        print(f"{ConsoleUI.BOLD}{ConsoleUI.CYAN}{'=' * width}{ConsoleUI.RESET}")
        print(
            f"  {ConsoleUI.GREEN}Thank you for using ChronoDownloader!{ConsoleUI.RESET}"
        )
        print(f"{ConsoleUI.BOLD}{ConsoleUI.CYAN}{'=' * width}{ConsoleUI.RESET}")
        print()
