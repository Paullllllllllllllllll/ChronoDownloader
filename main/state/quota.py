...
class ProviderQuota:
    ...
    def is_exhausted(self) -> bool:
        ...
        """
        return (
            self.downloads_used >= self.daily_limit and self.seconds_until_reset() > 0
        )
    ...
    def _check_and_reset_period(self, quota: ProviderQuota) -> bool:
        ...
        try:
            ...
            if hours_elapsed >= quota.reset_hours:
                ...
                logger.info(
                    "QUOTA RESET: %s - %d downloads now available (was %d/%d "
                    "used, %.1f hours elapsed)",
                    quota.provider_key,
                    quota.daily_limit,
                    old_used,
                    quota.daily_limit,
    ...
    def record_download(self, provider_key: str) -> int:
        ...
        with self._data_lock:
            ...
            if remaining <= 0 and not quota.exhausted_at:
                quota.exhausted_at = datetime.now(UTC).isoformat()
                logger.info(
    ...
    def get_quota_status(self, provider_key: str) -> dict[str, Any]:
        ...
        with self._data_lock:
            ...
            return {
                ...
                "reset_time": (
                    rt.isoformat() if (rt := quota.get_reset_time()) else None
                ),
