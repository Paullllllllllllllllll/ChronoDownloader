...
- QUOTA_LIMITED_PROVIDERS is the authoritative list of providers whose
  fast-download path is quota-gated
- _quota_preflight() consults main.state.quota before each download;
  raises QuotaDeferredException if the quota is exhausted and
  wait_for_reset is true. Caught by the existing deferred-queue handler
- _quota_record() records consumption after each successful download
  from a quota-backed provider
- Provider modules in api/providers/ are quota-agnostic; they do not
  import from main/

Adjacent packages consumed by this module:
- main.data: work directory naming, work.json CRUD, index.csv ledger
...
# Providers whose fast-download path is governed by a daily quota. The
# orchestration layer performs pre-flight quota checks and records
# consumption on success so that provider modules can remain quota-agnostic
# and free of any reverse dependency on main/.
QUOTA_LIMITED_PROVIDERS = frozenset({"annas_archive"})


def _provider_uses_quota_backed_api(pkey: str) -> bool:
    """Return True when the provider's fast-download path is active.

    A provider is considered quota-backed for the current call only when
    it is in :data:`QUOTA_LIMITED_PROVIDERS` *and* the credentials that
    enable its fast-download API are present. Without credentials the
    provider falls back to a non-quota-gated path and quota tracking does
    not apply.
    """
...
def _quota_preflight(pkey: str) -> None:
    """Raise :class:`QuotaDeferredException` if this provider's quota is spent.

    Consulted immediately before each ``download_func`` invocation for
    providers listed in :data:`QUOTA_LIMITED_PROVIDERS`. When the quota is
    exhausted and the provider's config sets ``wait_for_reset`` (default),
    the caller is expected to add the work to the deferred queue via its
    existing exception handler. When ``wait_for_reset`` is false, this
    function returns silently so the provider can use its fallback path.
    """
    ...
    quota_cfg = get_provider_setting(pkey, "quota", {}) or {}
    wait_for_reset = True
    if isinstance(quota_cfg, dict):
        wait_for_reset = bool(quota_cfg.get("wait_for_reset", True))

    if not wait_for_reset:
        logger.info(
            "%s quota exhausted but wait_for_reset=False; "
            "provider may use fallback path.",
            pkey,
        )
        return

    status = qm.get_quota_status(pkey) or {}
    reset_time_str = status.get("reset_time") if isinstance(status, dict) else None
    reset_time = None
    if reset_time_str:
    ...
    wait_hours = (wait_seconds or 86400.0) / 3600.0
    raise QuotaDeferredException(
        provider=pkey,
        reset_time=reset_time,
        message=f"{pkey}: quota exhausted. Resets in {wait_hours:.1f} hours.",
    )
