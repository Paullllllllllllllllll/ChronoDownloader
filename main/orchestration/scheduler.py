...
class ProviderSemaphoreManager:
    ...
    This ensures we don't overwhelm rate-limited providers even when using
    multiple workers.
    """
    ...
    def try_acquire(self, provider_key: str, timeout: float = 60.0) -> bool:
        """Acquire the provider's semaphore with a bounded wait.

        Used by fallback downloads, where the worker already holds another
        provider's semaphore: a bounded wait avoids the circular-wait deadlock
        that unbounded nested acquires could produce.
        ...
        Returns:
            True if the slot was acquired (caller must release), False if not.
        """
        sem = self._get_or_create(provider_key)
    ...
    def _get_or_create(self, provider_key: str) -> threading.Semaphore:
        ...
        with self._lock:
            if provider_key not in self._semaphores:
                ...
                logger.debug(
                    "Created semaphore for %s with limit %d", provider_key, limit
                )
...
# The semaphore manager of the currently-running scheduler, exposed so the
# pipeline's fallback downloads can honor the concurrency limit of the provider
# they ACTUALLY download from (the worker only holds the primary provider's
# slot). None when no parallel scheduler is active (sequential mode).
_ACTIVE_SEMAPHORE_MANAGER: ProviderSemaphoreManager | None = None
_ACTIVE_SEMAPHORE_LOCK = threading.Lock()


def get_active_semaphore_manager() -> ProviderSemaphoreManager | None:
    """Return the semaphore manager of the running scheduler, if any."""
    with _ACTIVE_SEMAPHORE_LOCK:
        return _ACTIVE_SEMAPHORE_MANAGER


def _set_active_semaphore_manager(mgr: ProviderSemaphoreManager | None) -> None:
    global _ACTIVE_SEMAPHORE_MANAGER
    with _ACTIVE_SEMAPHORE_LOCK:
        _ACTIVE_SEMAPHORE_MANAGER = mgr
...
class DownloadScheduler:
    ...
    def __init__(
        ...
        on_complete: Callable[[DownloadTask, bool, Exception | None], None]
        | None = None,
    ...
    def start(self) -> None:
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=self._max_workers, thread_name_prefix="dl_worker"
        )
        self._shutdown_event.clear()
        _set_active_semaphore_manager(self._semaphores)
        logger.info(
    ...
    def submit(
        self, task: DownloadTask, download_fn: Callable[[DownloadTask], bool]
    ) -> Future[Any] | None:
        ...
        if self._shutdown_event.is_set():
            logger.warning(
                "Scheduler shutting down; rejecting task for '%s'", task.title
            )
            return None
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.exception(
                        "Error executing download for '%s': %s", task.title, e
                    )
                    error = e
                    success = False
                finally:
            ...
            logger.exception(
                "Unexpected error in task runner for '%s': %s", task.title, e
            )
            error = e
            success = False
        finally:
    ...
    def wait_all(
    ...
    ) -> list[tuple[DownloadTask, bool, Exception | None]]:
        """Wait for all pending downloads to complete.

        Args:
            timeout: TOTAL maximum seconds to wait for the whole batch, not a
                per-task limit (None = wait indefinitely).

        Returns:
            List of (task, success, error) tuples. When the total timeout
            elapses, the results collected so far are returned and a warning
            is logged; tasks still running remain tracked by the scheduler.
        """
        ...
        try:
            ...
            logger.warning(
                "wait_all: total timeout (%.0fs) elapsed with %d task(s) still "
                "running; returning %d completed result(s).",
                timeout or 0.0,
                len(futures_copy) - len(results),
                len(results),
            )
